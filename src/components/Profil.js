...
export default class Profil extends Component {
  ...
  async setDataStorage() {
    ...
    data.customer
      ? this.setState({ typeUser: "customer", id: data.customer.id })
      : this.setState({ typeUser: "association", id: data.association.id });
  }
  ...
  fetchUser = async () => {
    ...
    try {
      const response = await fetch(
        `https://trocify.herokuapp.com/api/${this.state.typeUser}s/${this.state.id}`,
        settings
      );
      ...
      if (this.state.typeUser === "customer") {
        this.setState({
          tickets: json.data.customer["tickets"],
          user: json.data.customer,
          rank: json.data.customer["rank"],
        });
      } else {
  ...
  fetchAllAsso = async () => {
    ...
    try {
      const response = await fetch(
        "https://trocify.herokuapp.com/api/associations",
        settings
      );
      const json = await response.json();
  ...
  render() {
    ...
    let avatar =
      this.state.user.avatarFile === null
        ? "https://s3.amazonaws.com/uifaces/faces/twitter/adhamdannaway/128.jpg"
        : this.state.user.avatarFile;
        ...
        <View style={styles.view}>
          ...
          {typeUser == "customer" ? (
            <View style={styles.card}>
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate("MyDonations", { tickets: tickets })
                }
              >
                <Text>MES DONS</Text>
              </TouchableOpacity>
          ...
          {typeUser == "customer" ? (
            <View style={styles.card}>
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate("Rank", { user: user, token: token })
                }
              >
                <Text>MON RANK</Text>
              </TouchableOpacity>
          ...
          {typeUser == "customer" ? (
            <View style={styles.card}>
              ...
              <TouchableOpacity
                onPress={() =>
                  navigation.navigate("History", { tickets: tickets })
                }
              >
